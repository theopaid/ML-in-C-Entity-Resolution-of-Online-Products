...
double *train_weights(HashTable *hash_table, HashTable_w *W1) {
    ...
    while ( threshold < 0.5 ) {
        //b = thrd_model_training_wghts(hash_table, W1, b); // !THREADS
...
Observation *get_pair_in_x_excl_set(HashTable *hash_table, HashTable_w *W1) {
    ...
    new_pair->left_tf_idf = randm_spc1->tf_idfVectorFinal;
    new_pair->right_spec_id = randm_spc2->specId;
    new_pair->right_tf_idf = randm_spc2->tf_idfVectorFinal;
    new_pair->is_match = -1;
...
PairInfo_w *get_first_pair_w(HashTable_w *W1) {
    if ( W1 == NULL || W1->hashArray == NULL || W1->size == 0 ) return NULL;
    for( int i = 0 ; i < W1->size ; i++ ) {
...
Vector *get_spec_tf_idf_vector(SpecNode *spec_node) {
    if ( spec_node == NULL ) return NULL;
    return spec_node->cliquePtr->specInfo->tf_idfVectorFinal;
}